...
describe('resource endpoints', () => {
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.endpoints.list({
    mine: true,
    type: 'dedicated',
  ...
  test('listHardware', async () => {
    const responsePromise = client.endpoints.listHardware();
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('listHardware: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.endpoints.listHardware({ model: 'deepseek-ai/DeepSeek-R1' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Together.NotFoundError);
  });
});
