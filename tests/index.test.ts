...
import { APIPromise } from 'together-ai/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['TOGETHER_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Together) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Together({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'TOGETHER_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    test('empty env variable', () => {
      ...
      const client = new Together({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.together.ai/v1');
    });
